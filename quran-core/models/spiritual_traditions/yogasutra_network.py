...
class YogasutraNetwork(nn.Module):
    ...
    def self_test(cls) -> bool:
        ...
        model = cls(vocab_size=100, sutra_dim=32, hidden_dim=64)
        model.eval()
        x = torch.randint(0, 100, (2, 8))
        with torch.no_grad():
            result = model(x)
        assert result['kaivalya'].shape == (2, 1), f"kaivalya shape {result['kaivalya'].shape}"
        assert result['samadhi_result']['samadhi_depth'].shape == (2, 1)
        assert result['sutra'].shape == (2, 32)
        print("YogasutraNetwork self_test PASSED")
        return True
