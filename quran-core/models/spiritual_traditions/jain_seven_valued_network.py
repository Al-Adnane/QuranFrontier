...
class ConditionalPredicator(nn.Module):
    ...
    def __init__(self, embed_dim: int = 256):
        ...
        # Conditional truth modifier (7 truth probs + embed_dim context)
...
class JainSevenValuedNetwork(nn.Module):
    ...
    def forward(
        ...
        # Get the full embedded representation for perspective taking
        embedded = self.embedding.embed(x).mean(dim=1)

        # Take multiple perspectives on the full embedding
        perspective_result = self.perspective_taker(embedded)
    ...
    def self_test(cls) -> bool:
        ...
        with torch.no_grad():
            result = model(x)
        assert result['syadvada'].shape == (2, 7), f"syadvada shape {result['syadvada'].shape}"
        assert result['truth_state'].probabilities.shape == (2, 7)
        assert len(result['truth_values']) == 7
        print("JainSevenValuedNetwork self_test PASSED")
        return True
