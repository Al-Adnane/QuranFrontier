...
class MentalSpaceEncoder(nn.Module):
    ...
    def forward(
        ...
        # Relations: use raw tensor as relation scores [batch, num_elements, num_elements]
        relations = relation_ids.float()
...
class BlendingNetwork(nn.Module):
    ...
    def _compute_blend_quality(
        ...
        webbing = blended.relations.abs().mean(dim=-1).mean(dim=-1, keepdim=True)
...
class ConceptualBlendingNetwork(nn.Module):
    ...
    def self_test(cls) -> bool:
        ...
        model = cls(vocab_size=100, element_dim=32, max_elements=5)
        model.eval()
        batch = 2
        n_elem = 5
        ...
        print("ConceptualBlendingNetwork self_test PASSED")
        return True
