...
class PredictiveCodingNetwork(nn.Module):
    ...
    def self_test(cls) -> bool:
        ...
        model = cls(input_dim=32, hidden_dims=[64, 32, 16, 8], action_dim=4)
        model.eval()
        x = torch.randn(2, 32)
        with torch.no_grad():
            result = model(x)
        assert result['free_energy'].shape == (2, 1), f"FE shape {result['free_energy'].shape}"
        assert len(result['hidden_states']) == 4
        assert result['hidden_states'][-1].shape == (2, 8)
        print("PredictiveCodingNetwork self_test PASSED")
        return True
