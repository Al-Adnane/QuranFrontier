...
class ShintoMusubiNetwork(nn.Module):
    ...
    def self_test(cls) -> bool:
        ...
        with torch.no_grad():
            result = model(x)
        assert result['harmony'].shape == (2, 1), f"harmony shape {result['harmony'].shape}"
        assert result['harai']['purity'].shape == (2, 1)
        assert result['flow']['becoming_score'].shape == (2, 1)
        print("ShintoMusubiNetwork self_test PASSED")
        return True
