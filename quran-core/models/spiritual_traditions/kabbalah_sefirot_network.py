...
class KabbalahSefirotNetwork(nn.Module):
    ...
    def self_test(cls) -> bool:
        ...
        assert result['shekhinah'].shape == (2, 1), f"shekhinah shape {result['shekhinah'].shape}"
        assert len(result['sefirot_states']) == 10
        assert result['sefirot_states'][0].shape == (2, 64)
        print("KabbalahSefirotNetwork self_test PASSED")
        return True
