...
class IntegratedInformationNetwork(nn.Module):
    ...
    def self_test(cls) -> bool:
        ...
        model = cls(input_dim=32, hidden_dim=64, num_units=16)
        model.eval()
        x = torch.randn(2, 10, 32)
        with torch.no_grad():
            result = model(x, compute_phi=True)
        assert result['final_state'].shape == (2, 16), f"state shape {result['final_state'].shape}"
        assert result['phi'] is not None
        assert result['phi'].shape == (2,), f"phi shape {result['phi'].shape}"
        assert result['integration_score'].shape == (2, 1)
        print("IntegratedInformationNetwork self_test PASSED")
        return True
