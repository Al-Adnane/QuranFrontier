...
class IChingNetwork(nn.Module):
    ...
    def self_test(cls) -> bool:
        ...
        model = cls(input_dim=64, hex_dim=64, hidden_dim=128)
        model.eval()
        x = torch.randn(2, 64)
        with torch.no_grad():
            result = model(x)
        assert result['judgment'].shape == (2, 1), f"judgment shape {result['judgment'].shape}"
        assert result['hex_state'].lines.shape == (2, 6)
        assert result['trigram_result']['upper_index'].shape == (2,)
        print("IChingNetwork self_test PASSED")
        return True
