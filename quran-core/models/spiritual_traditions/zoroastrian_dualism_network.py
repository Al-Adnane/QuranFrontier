...
class ZoroastrianDualismNetwork(nn.Module):
    ...
    def self_test(cls) -> bool:
        ...
        with torch.no_grad():
            result = model(x)
        assert result['asha'].shape == (2, 1), f"asha shape {result['asha'].shape}"
        assert result['battle']['outcome'].shape == (2, 1)
        assert result['guardians']['protection'].shape == (2, 1)
        print("ZoroastrianDualismNetwork self_test PASSED")
        return True
