...
class PolynesianWayfindingNetwork(nn.Module):
    ...
    def __init__(self, embed_dim: int = 256):
        ...
        # Navigation integration (32 houses + 8 sea patterns + 8 winds + embed_dim etak)
        self.nav_integration = nn.Linear(32 + 8 + 8 + embed_dim, embed_dim)
    ...
    def forward(
        ...
        # Integrate navigation signals
        nav_integrated = self.nav_integration(nav_input)

        # Navigation confidence
        confidence = self.confidence(nav_integrated)
    ...
    def self_test(cls) -> bool:
        ...
        assert result['confidence'].shape == (2, 1), f"conf shape {result['confidence'].shape}"
        assert result['etak']['position'].shape == (2, 2)
        print("PolynesianWayfindingNetwork self_test PASSED")
        return True
